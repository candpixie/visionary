...
def navigate_once(encoded=False) -> Optional[str]:
    """Listen for a destination, process it with Claude, and navigate once."""
    print("🎤 Listening for destination...")
    speak("Where would you like to go?")
    
    try:
        # Step 1: Listen for audio (destination)
        transcription_result = analyzer.listen_for_audio()
        destination_query = transcription_result.text.strip()
        print(f"🗣️  Heard destination: {destination_query}")
        
        # Step 2: Process destination using Claude
        formatted_destination = process_destination_with_claude(destination_query)
        if not formatted_destination:
            speak("I couldn't understand the destination. Please try again.")
            return None
        
        print(f"📍 Destination confirmed: {formatted_destination}")
        speak(f"Navigating to {formatted_destination}")
        time.sleep(1)
        
        # Step 3: Start navigation (single route run)
        navigate_to_destination(formatted_destination)
        ...
        print(error_msg)
        speak("Sorry, something went wrong during navigation.")
        return None
...
def main():
        if len(sys.argv) > 1 and sys.argv[1] == "--once":
            ...
            print("Usage: python navigation.py --once")
            print("Runs one navigation session with speech + Claude integration.")
